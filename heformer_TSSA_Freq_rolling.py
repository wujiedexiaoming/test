...
Encoder: PE ─┬─ FreqBlock (120步→61频率bin, FFT增强) ─┐
             └─ Identity ─────────────────────────────┴─ + ─→ TSSA × 2
Decoder: TSSA Self + MHA Cross
"""
...
OUTPUT_DIR = 'heformer_Freq_rolling'
...
# ==================== 3.5. FreqBlock 频域增强 (PE 之后) ====================

class FreqBlock(nn.Module):
    """FFT 频域增强分支: FFT → MLP幅度增强 → IFFT"""
    def __init__(self, d_model, seq_len=120, dropout=0.1):
        super().__init__()
        self.d_model = d_model
        self.n_freqs = seq_len // 2 + 1  # 61
    ...
    def forward(self, x):
        ...
        X = torch.fft.rfft(x_t, dim=-1)                # [B, C, F], F=61
        ...
        x_out_t = torch.fft.irfft(X_enhanced, n=L, dim=-1)
        x_out = x_out_t.transpose(1, 2)

        return torch.tanh(self.gate) * x_out
...
class RollingWarningTransformer(nn.Module):
    ...
    def __init__(self, input_dim, dec_seq_len, d_model=256, nhead=8,
        ...
        # FreqBlock 频域增强分支 (PE 之后)
        self.freq_block = FreqBlock(d_model, seq_len=120, dropout=freq_dropout)
    ...
    def forward(self, src, tgt, is_training=True):
        # Encoder: PE → 并行分支(FreqBlock+Identity) → TSSA
        src_emb = self.pos_encoding(self.encoder_input_layer(src))
        freq_out = self.freq_block(src_emb)
        src_emb = src_emb + freq_out  # 残差融合, FreqBlock学不到就走Identity
...
if __name__ == '__main__':
    ...
    print(f"Params: {sum(p.numel() for p in model.parameters()):,} (FreqBlock: {freq_params:,})")
    print(f"FreqBlock: {model.freq_block.n_freqs} freq bins, gate={model.freq_block.gate.item():.4f}")
