# -*- coding: utf-8 -*-
"""
Informer风格 + TSSA（stride=12加速版）
- Encoder Self-Attention: TSSA (O(n)复杂度)
- Decoder Self-Attention: TSSA (O(n)复杂度)
- Decoder Cross-Attention: 标准MultiheadAttention (保证效果)
- stride=12: 窗口间隔1分钟，减少冗余样本，加速训练
"""
...
DIM_FEEDFORWARD = 256   #可优化
DROPOUT = 0.1
BATCH_FIRST = True

BATCH_SIZE = 32
EPOCHS = 100
LEARNING_RATE = 1e-3    #可优化
WEIGHT_DECAY = 1e-4
PATIENCE_LR = 3
...
WINDOW_STRIDE = 1  # 窗口间隔1分钟，减少冗余

OUTPUT_DIR = 'heformer_tssa_fast'  # stride=12加速版
...
def load_and_preprocess_data(file_path, target_column, train_ratio, val_ratio, test_ratio,
    ...
    print("=" * 60)
    print(f"【TSSA混合架构】输入{enc_seq_len}步 -> 输出{dec_seq_len}步")
    print("=" * 60)
...
class HybridTSSATransformer(nn.Module):
    """
    混合架构：
    - Encoder: TSSA (线性复杂度)
    - Decoder Self: TSSA (线性复杂度)
...
if __name__ == '__main__':
    ...
    print(f"\n完成！结果在 {OUTPUT_DIR}")
    print("如果效果仍不如标准Transformer，说明TSSA不适合你的时序数据，建议回退到RevIN+标准Transformer")
