# -*- coding: utf-8 -*-
"""
TSSA + 膨胀因果卷积 (Dilated Causal TCN) 混合架构

Encoder:
  Linear → Causal TCN (3层, dil=1/2/4) → PositionalEncoding → TSSALayer × 2
...
OUTPUT_DIR = 'heformer_tssa_tcn_prediction'
...
class TCN_TSSA_Transformer(nn.Module):
    """
    Encoder: Linear → Causal TCN → PE → TSSALayer × N
    ...
    def forward(self, src, tgt, is_training=True):
        # ===== Encoder: Linear → Causal TCN → PE → TSSA =====
        src_emb = self.encoder_input_layer(src)          # [B, 24, d_model]
        src_emb = self.tcn_encoder(src_emb)               # [B, 24, d_model]  因果, 无未来泄漏
...
def load_and_preprocess_data(file_path, target_column, train_ratio, val_ratio, test_ratio,
    ...
    print("=" * 60)
    print(f"【TCN-TSSA混合架构】输入{enc_seq_len}步 -> 输出{dec_seq_len}步")
    print("=" * 60)
...
if __name__ == '__main__':
    ...
    print(f"架构: TCN({TCN_KERNEL_SIZE}, dil=1/2/4, causal) → TSSA × {NUM_ENCODER_LAYERS} → HybridDecoder × {NUM_DECODER_LAYERS}")
