...
# 【关键】Informer风格：输入24步，直接输出24步（非自回归）
ENC_SEQ_LEN = 24
DEC_SEQ_LEN = 36
...
class TimeSeriesDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tgt = torch.zeros(self.dec_seq_len + 1, 1)  # +1 是 start_token
        tgt[0, 0] = start_token_val  # 第一个位置放历史最后一个值
...
def load_and_preprocess_data(file_path, target_column, train_ratio, val_ratio, test_ratio, 
    ...
    print("=" * 60)
    print(f"【Informer风格直接预测】输入{enc_seq_len}步 -> 直接输出{dec_seq_len}步")
    print("=" * 60)
...
class InformerStyleTransformer(nn.Module):
    ...
    Informer风格的直接多步预测 Transformer
    关键特点：
    1. 训练时使用 Teacher Forcing（传入真实值）
    2. 预测时输入 [start_token + zeros]，一次性输出所有预测步
    3. 非自回归，不需要逐步生成
    """
    def __init__(self, input_dim, dec_seq_len, batch_first=True,
        ...
        self.decoder_input_layer = nn.Linear(1, d_model)  # 目标值是单变量
        ...
        self.transformer = nn.Transformer(
            ...
            activation='gelu'  # Informer使用gelu
        )
    ...
    def make_causal_mask(self, sz):
        ...
        return mask  # True表示要mask掉的位置
    
    def make_inference_mask(self, sz):
        """
        生成推理掩码（Informer风格）
        关键：start_token可以看到自己，预测位置可以看到start_token
        但预测位置之间不互相mask（非自回归）
        """
        # 创建一个全0掩码（不mask任何位置）
        # 因为我们直接预测，不需要因果约束
    ...
    def forward(self, src, tgt, is_training=True):
        ...
        Args:
            ...
            tgt: [batch, dec_seq_len+1, 1] 解码器输入 [start_token + values]
            is_training: 是否训练模式
        """
        ...
        if not self.batch_first:
            src_embed = src_embed.transpose(0, 1)  # [seq, batch, dim]
        ...
        if is_training:
            # 训练时：使用因果掩码（Teacher Forcing）
            tgt_mask = self.make_causal_mask(tgt_len).to(src.device)
        else:
            # 推理时：不使用因果掩码（Informer风格直接预测）
            # 允许模型一次性看到所有位置
            tgt_mask = None  # 或者使用 make_inference_mask
        ...
        output = self.transformer(
            ...
            tgt_embed,
            tgt_mask=tgt_mask,
            tgt_is_causal=is_training  # 推理时不使用因果注意力
        )
        ...
        # 只取预测部分（去掉start_token对应的位置）
        # tgt输入是 [start_token, step1, step2, ..., stepN]
        # 输出对应位置也是 [pred_for_start, pred_for_step1, ..., pred_for_stepN-1]
        # 但实际上我们只关心后dec_seq_len个位置的输出
        if is_training:
            # 训练时：tgt包含真实值，我们预测的是下一个位置
            # 输出 [0:-1] 对应 tgt [1:] 的预测
            return output[:, :-1, :] if self.batch_first else output[:-1, :, :]
        else:
            # 推理时：输入是 [start_token, 0, 0, ...]
            # 输出 [:, 1:, :] 是各步的预测
            return output[:, 1:, :] if self.batch_first else output[1:, :, :]
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device, epochs, scaler_dict):
    ...
    for epoch in range(epochs):
        ...
        for src, tgt, target in train_loader:
            ...
            # 训练时使用 Teacher Forcing（is_training=True）
            # tgt包含 [start_token + 真实值]
        ...
        with torch.no_grad():
            for src, tgt, target in val_loader:
                ...
                # 【关键】验证时使用 is_training=False
                # 模型会忽略 tgt 中的真实值（除start_token外），直接预测
