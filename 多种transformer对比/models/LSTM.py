# -*- coding: utf-8 -*-
"""LSTM — 双层 LSTM Encoder-Decoder 直接多步预测"""
import sys, os
...
class LSTMEncoderDecoder(nn.Module):
    def __init__(self, input_dim, dec_seq_len, d_model=256, num_layers=2, dropout=0.1):
        ...
        self.encoder = nn.LSTM(input_dim, d_model, num_layers, dropout=dropout, batch_first=True)
        self.decoder = nn.LSTM(1, d_model, num_layers, dropout=dropout, batch_first=True)
...
def main():
    ...
    model = LSTMEncoderDecoder(input_dim, DEC_SEQ_LEN, d_model=D_MODEL, num_layers=2, dropout=DROPOUT).to(device)
    print(f"\n[LSTM] Params: {sum(p.numel() for p in model.parameters()):,}")
    model = train_model(model, train_loader, val_loader, device, 'LSTM')
    results = predict_full_series(model, test_loader, scaler_dict, device)
    print(f"[LSTM] Test RMSE={results['rmse']:.4f} MAE={results['mae']:.4f} R²={results['r2']:.4f}")
    np.save(f'{PREDICTIONS_DIR}/LSTM_pred.npy', results['pred'])
    np.save(f'{PREDICTIONS_DIR}/LSTM_true.npy', results['true'])
    return results
